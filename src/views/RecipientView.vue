...
async function loadConfession() {
  ...
  catch { error.value = "Link confess tidak valid atau sudah tidak tersedia." }
...
async function handleResponse(response) {
  ...
  catch { error.value = "Gagal mengirim jawaban. Silakan coba lagi." }
...
<template>
  ...
  >
    ...
    <!-- Error -->
    <div
      v-else-if="error && !confession"
      class="relative flex min-h-screen items-center justify-center px-5"
    >
